...
const Trans_Rate = Models.Trans_Rate;
const Transaction = Models.Transaction;
module.exports = {
    create : function (accessUserId,accessUserType, transData, callback) {
        try {
            let queryObj = {};
            queryObj.transactionid=transData.transactionid;
            queryObj.rate = transData.rate;
            queryObj.createdBy = accessUserId;
            queryObj.updatedBy = accessUserId;
            Transaction.findOne({where : {id:transData.transactionid}}).then(result=>{
                "use strict";
                if (result.dataValues.buyer===accessUserId)
                {
                    queryObj.userid=result.dataValues.seller;
                    Trans_Rate.create(queryObj).then(result=>{
                        "use strict";
                        return callback(null, null, 200, null, result);
                    }).catch(function(error){
                        "use strict";
                        return callback(1, 'create_trans_rate_fail', 400, error, null);
                    });
                }
                else 
                {
                    queryObj.userid=result.dataValues.buyer;
                    Trans_Rate.create(queryObj).then(result=>{
                        "use strict";
                        return callback(null, null, 200, null, result);
                    }).catch(function(error){
            ...
            return callback(2, 'create_trans_rate_fail', 400, error, null);
        }
    },
